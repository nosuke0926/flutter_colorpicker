...
class HSVWithValueColorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      ...
      size.height * 0.07,
      Paint()
        ..color = pointerColor ?? Colors.white70
        ..strokeWidth = 6.0
        ..style = PaintingStyle.stroke,
    );
  }
...
class _SliderLayout extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    positionChild(thumb, Offset(0.0, size.height * 0.44));
    layoutChild(
      gestureContainer,
...
class ColorPickerInputModal extends StatefulWidget {
  const ColorPickerInputModal(
    this.modalTitle,
    this.color,
    this.onColorChanged, {
    Key? key,
  ...
  @override
  State<ColorPickerInputModal> createState() => _ColorPickerInputModalState();
}

class _ColorPickerInputModalState extends State<ColorPickerInputModal> {
  TextEditingController textEditingController = TextEditingController();
  int inputColor = 0;
